...
const getRoleTag = (role) => {
  ...
  switch (roleLower) {
    case 'rm': color = 'purple'; break;
    case 'deferral management': color = 'green'; break;
    case 'creator': color = 'green'; break;
    case 'co_checker': color = 'volcano'; break;
    case 'system': color = 'default'; break;
    default: color = 'blue';
  }
  return <Tag color={color} style={{ marginLeft: 8, textTransform: 'uppercase' }}>{roleLower.replace(/_/g, ' ')}</Tag>;
};
        ...
        renderItem={(item) => (
          <List.Item>
            <List.Item.Meta
              ...
              title={<div style={{ display: 'flex', justifyContent: 'space-between' }}><div><b>{formatUsername(item.user) || 'System'}</b>{item.userRole ? getRoleTag(item.userRole) : null}</div><span style={{ fontSize: 12, color: '#999' }}>{dayjs(item.date).format('DD MMM YYYY HH:mm')}</span></div>}
              description={<div className="break-words">{item.comment || item.notes || 'No comment provided.'}</div>}
            />
          </List.Item>
        )}
      />
...
const DeferralReviewModal = ({ deferral, open, onClose, onDecision }) => {
  ...
  useEffect(() => { setLocalDeferral(deferral); }, [deferral]);
  ...
  const getAllDocuments = () => {
    const all = [];
    (localDeferral.attachments || []).forEach((att, i) => { const isDCL = att.name && att.name.toLowerCase().includes('dcl'); all.push({ id: att.id || `att_${i}`, name: att.name, type: getFileExtension(att.name || ''), url: att.url, isDCL, isUploaded: true, source: 'attachments', uploadDate: att.uploadDate }); });
    (localDeferral.additionalFiles || []).forEach((f, i) => { all.push({ id: `add_${i}`, name: f.name, type: getFileExtension(f.name || ''), url: f.url, isAdditional: true, isUploaded: true, source: 'additionalFiles' }); });
    (localDeferral.selectedDocuments || []).forEach((d, i) => { all.push({ id: `req_${i}`, name: typeof d === 'string' ? d : d.name || d.label || 'Document', type: d.type || '', isRequested: true, isSelected: true, source: 'selected' }); });
    (localDeferral.documents || []).forEach((d, i) => { all.push({ id: d.id || `doc_${i}`, name: d.name, type: d.type || getFileExtension(d.name || ''), url: d.url, isDocument: true, isUploaded: !!d.url, source: 'documents' }); });
    return all;
  };
  ...
  const approverEmail = (
    ...
    (localDeferral?.approverFlow && (localDeferral.approverFlow[0]?.email || localDeferral.approverFlow[0]?.user?.email)) ||
    (localDeferral?.approvers && (localDeferral.approvers[0]?.email || (typeof localDeferral.approvers[0] === 'string' && localDeferral.approvers[0].includes('@') && localDeferral.approvers[0]))) ||
    null
  );

  const facilityColumns = [
    { title: 'Facility Type', dataIndex: 'facilityType', key: 'facilityType', render: (t) => <Text strong>{t || 'N/A'}</Text> },
    { title: "Sanctioned (KES '000)", dataIndex: 'sanctioned', key: 'sanctioned', align: 'right', render: (v, r) => { const val = v ?? r.amount ?? 0; return Number(val || 0).toLocaleString(); } },
    ...
    try {
      ...
      if (r && r.success) {
        ...
        try {
          ...
          try { window.dispatchEvent(new CustomEvent('deferral:updated', { detail: updated })); } catch(e) { /* ignore */ }
  ...
  const isCurrentUserApprover = (() => {
    ...
    if (appr && appr.user) {
      ...
      return currentUser && ((currentUser._id || currentUser.id).toString() === approverId);
    }
    const first = (localDeferral.approverFlow && localDeferral.approverFlow[0]) || (localDeferral.approvers && localDeferral.approvers[0] && (localDeferral.approvers[0].email || localDeferral.approvers[0].name));
    if (typeof first === 'string') {
      return (currentUser?.email && first.includes(currentUser.email)) || (currentUser?.name && first.includes(currentUser.name));
    }
    return false;
  })();
  ...
  const handleApprove = async () => {
    setApproving(true);
    try {
      const updated = await deferralApi.approveDeferral(localDeferral._id, token);
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          ...
          <p><strong>{localDeferral?.deferralNumber}</strong> - {localDeferral?.customerName}</p>
          <p>Please provide a reason for rejection:</p>
          <Input.TextArea rows={3} placeholder="Enter rejection reason..." style={{ marginTop: 8 }} id={`rejectionComment_${localDeferral?._id}`} />
        </div>
      ),
      okText: 'Yes, Reject',
      okType: 'danger',
      ...
      cancelText: 'Cancel',
      async onOk() {
        const commentInput = document.getElementById(`rejectionComment_${localDeferral?._id}`);
        ...
        setRejecting(true);
        try {
          const updated = await deferralApi.rejectDeferral(localDeferral._id, comment, token);
  ...
  if (isCurrentUserApprover && isPendingApproval && !isWithdrawn) {
    footerButtons.push(<Popconfirm key="confirmReject" title="Are you sure you want to reject this deferral?" onConfirm={handleReject} okText="Reject" cancelText="Cancel"><Button key="reject" danger loading={rejecting}>Reject</Button></Popconfirm>);
    footerButtons.push(<Button key="approve" type="primary" onClick={handleApprove} loading={approving} style={{ backgroundColor: PRIMARY_BLUE, borderColor: PRIMARY_BLUE }}>Approve</Button>);
  } else if (isWithdrawn) {
    footerButtons.push(<Button key="reject" danger disabled style={{ backgroundColor: '#d9d9d9', borderColor: '#d9d9d9', color: '#8c8c8c' }}>Reject</Button>);
    footerButtons.push(<Button key="approve" type="primary" disabled style={{ backgroundColor: '#d9d9d9', borderColor: '#d9d9d9', color: '#8c8c8c' }}>Approve</Button>);
  }
  footerButtons.push(<Button key="close" onClick={onClose}>Close</Button>);

  return (
    <>
      <style>{customStyles}</style>
      <Modal title={<div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><BankOutlined /> <span>Deferral Request: {localDeferral.deferralNumber}</span></div>} open={open} onCancel={onClose} width={950} styles={{ body: { padding: '0 24px 24px' } }} footer={footerButtons}>
        ...
        <Card className="deferral-info-card" size="small" title={<span style={{ color: PRIMARY_BLUE }}>Deferral Details</span>} style={{ marginBottom: 18 }}>
          <Descriptions size="middle" column={{ xs: 1, sm: 2, lg: 3 }}>
            ...
            <Descriptions.Item label="Next Due Date">
              <div style={{ color: (localDeferral.nextDueDate || localDeferral.nextDocumentDueDate || localDeferral.requestedExpiry) ? (dayjs(localDeferral.nextDueDate || localDeferral.nextDocumentDueDate || localDeferral.requestedExpiry).isBefore(dayjs()) ? ERROR_RED : SUCCESS_GREEN) : PRIMARY_BLUE }}>
                {(localDeferral.nextDueDate || localDeferral.nextDocumentDueDate || localDeferral.requestedExpiry) ? `${dayjs(localDeferral.nextDueDate || localDeferral.nextDocumentDueDate || localDeferral.requestedExpiry).format('DD MMM YYYY')}` : 'Not calculated'}
              </div>
            </Descriptions.Item>
        ...
        <Card size="small" title={<span style={{ color: PRIMARY_BLUE }}>Mandatory: DCL Upload {dclDocs.length > 0 ? '✓' : ''}</span>} style={{ marginBottom: 18 }}>
          {dclDocs.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              <div style={{ padding: 8, backgroundColor: '#f6ffed', borderRadius: 4, marginTop: 8 }}><Text type="success" style={{ fontSize: 12 }}>✓ {dclDocs.length} DCL document{dclDocs.length !== 1 ? 's' : ''} uploaded successfully</Text></div>
            </div>
          ) : (
            <div style={{ textAlign: 'center', padding: 16, color: WARNING_ORANGE }}><UploadOutlined style={{ fontSize: 24, marginBottom: 8, color: WARNING_ORANGE }} /><div>No DCL document uploaded</div><Text type="secondary" style={{ fontSize: 12, display: 'block', marginTop: 4 }}>DCL document is required for submission</Text></div>
          )}
        </Card>
        ...
        <Card size="small" title={<span style={{ color: PRIMARY_BLUE }}><PaperClipOutlined style={{ marginRight: 8 }} /> Additional Uploaded Documents ({uploadedDocs.length})</span>} style={{ marginBottom: 18 }}>
          {uploadedDocs.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {uploadedDocs.map((doc, i) => (
                <div key={doc.id || i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', backgroundColor: '#f8f9fa', borderRadius: 6, border: '1px solid #e8e8e8' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    {getFileIcon(doc.type)}
                    <div>
                      <div style={{ fontWeight: 500, fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>{doc.name}<Tag color="blue" style={{ fontSize: 10 }}>Uploaded</Tag></div>
                      <div style={{ fontSize: 12, color: '#666', display: 'flex', gap: 12, marginTop: 4 }}>{doc.size && (<span>{doc.size > 1024 ? `${(doc.size/1024).toFixed(2)} MB` : `${doc.size} KB`}</span>)}{doc.uploadDate && (<span>Uploaded: {dayjs(doc.uploadDate).format('DD MMM YYYY HH:mm')}</span>)}{doc.isAdditional && (<Tag color="cyan" style={{ fontSize: 10, padding: '0 4px' }}>Additional</Tag>)}</div>
                    </div>
                  </div>
                  <Space>
                    {doc.url && (<Button type="text" icon={<EyeOutlined />} onClick={() => openFileInNewTab(doc.url)} size="small">View</Button>)}
              ...
              <div style={{ padding: 8, backgroundColor: '#f6ffed', borderRadius: 4, marginTop: 8 }}><Text type="success" style={{ fontSize: 12 }}>✓ {uploadedDocs.length} document{uploadedDocs.length !== 1 ? 's' : ''} uploaded</Text></div>
            </div>
          ) : (
            <div style={{ textAlign: 'center', padding: 16, color: '#999' }}><PaperClipOutlined style={{ fontSize: 24, marginBottom: 8, color: '#d9d9d9' }} /><div>No additional documents uploaded</div><Text type="secondary" style={{ fontSize: 12, display: 'block', marginTop: 4 }}>You can upload additional supporting documents if needed</Text></div>
          )}
        </Card>

        <Card size="small" title={<div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div><span style={{ color: PRIMARY_BLUE, fontSize: 14 }}>Approval Flow {isPendingApproval && (<Tag color="orange" style={{ marginLeft: 8, fontSize: 11 }}>Pending Approval</Tag>)}</span></div>
            {isPendingApproval ? (
              <div style={{ marginLeft: 12 }}>
                <Button
                  ...
                  type="default"
                  style={{ borderColor: ACCENT_LIME, color: PRIMARY_BLUE, fontWeight: 600 }}
                >
                  Send Reminder
                </Button>
              </div>
...
const DeferralPending = ({ userId = "creator_current" }) => {
  ...
  const [tableHeight, setTableHeight] = useState(
    typeof window !== 'undefined' ? Math.max(300, window.innerHeight - 480) : 600
  );
  ...
  const filteredData = useMemo(() => {
    let filtered = deferrals.filter((d) => d.status === "pending_creator_review");
  ...
  const handleDecision = async (decisionData) => {
    setLoading(true);
    setTimeout(() => {
      setDeferrals(prev => prev.filter(d => d._id !== selectedDeferral._id));
      setLoading(false);
      setModalOpen(false);
